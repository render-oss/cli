...
func TestResolveConsent(t *testing.T) {
	testCases := []struct {
		...
		{name: "enabled by default with no opt-out env vars"},
		...
		{name: "both opt-out env vars set stays opted out", doNotTrack: "1", disableEnv: "1", wantReason: OptOutReasonDoNotTrack},
	}
